...
object Record {
  ...
  implicit class RecordOps[R <: Record](val r: R) extends AnyVal {
    // TODO: since the dotty lib is compiled with Scala 2.12, we have to use `with` instead of intersection types
    def update[V](label: String, value: V)
                 (implicit
                  ub: UpperBoundUpdater[R, label.type, V],
                  poly: PolymorphicUpdater[R, label.type, V]): ub.Out with poly.Out =
      new Record(r._data + (label -> value)).asInstanceOf[ub.Out with poly.Out]
  }
}